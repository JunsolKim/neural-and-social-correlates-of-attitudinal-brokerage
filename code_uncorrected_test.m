...
%betweenness_z	closeness_z	eigenvector_z	
...
all_behav = zscore(data{valid,behav_name}); %set all_behav
...
% Moderating analysis
load('data/all_mats_161.mat'); %load all_mats
network_90 = zeros(227,227);
network_90(find(sum_pos_mask > 139*0.9)) = 1;
brainconn = sum(sum(all_mats.*network_90.*social_brain_mask));
brainconn = reshape(brainconn, [], size(brainconn, 3));
brainconn = brainconn';
b = fitlm([zscore(brainconn(valid)), zscore(data{valid, 'n_constraint'}), zscore(brainconn(valid)).*zscore(data{valid, 'n_constraint'})], data{valid, behav_name})
